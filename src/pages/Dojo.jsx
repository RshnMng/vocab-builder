...
export default function Dojo (props){
    ...
    if(maxShown === 6){
        ...
        newDisplay.push(<div key={createRandomId()} className={'nym-choices'}>{i}</div>);
    }}
    else if(maxShown === 12){
    ...
    function handleShownAmount(){
       ...
       setMaxShown((prevState) => {
        ...
        function acceptLoss(){
       dojoStorage.round === 1 && setDojoStorage((prevState) => {
            return {...prevState, incorrect: 0, round: 1, points: prevState.points - (Math.floor(prevState.points * .25))}
        });
        
       dojoStorage.round === 2 && setDojoStorage((prevState) => {
            return {...prevState, incorrect: 0, round: 1, points: prevState.points - (Math.floor(prevState.points * .5))}
        });
        
       dojoStorage.round === 3 && setDojoStorage((prevState) => {
 ...
 return <> <div className='row'>
                ...
                {wrong && <div className='wrong-div'>
                            <audio id='audioPlayer' src='/216090__richerlandtv__bad-beep-incorrect.mp3' type='audio/mp3'>
                            </audio>
                        </div>}
