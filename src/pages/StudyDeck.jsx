...
export default function StudyDeck(props){
    ...
    function displaySavedSyns(){
       return deckInfo[index].same.map((syn) => {
            return <Link onClick={(event) => handleNymButton(event)} to={editing ? '#' : '/dictionary'} key={createRandomId()}><button  className={editing ? 'study-deck-show-syn edit' : 'study-deck-show-syn'} id={syn}>
            {syn}
     ...
     function displaySavedAnts(){
       return deckInfo[index].opposite.map((ant) => {
            return <Link onClick={(event) => handleNymButton(event)} to={editing ? '#' : '/dictionary'} key={createRandomId()}> <button key={createRandomId()} className={editing ? 'study-deck-show-ant edit' : 'study-deck-show-ant'} id={ant}>
            {ant}
    ...
    function handleDuplicates(){
       ...
       return duplicate;
        ...
        let strugglePopup = <div className='add-struggle-popup'>
                            ...
                            <button className='add-struggle-btn' onClick={() => {
                                resetCorrectCount()
                                increaseIndex()
                                isStuggleDuplicate(false)
           ...
           let alreadyAddedPopup = <div style={{'marginBottom': '30px'}} className='add-struggle-popup'>
    ...
    useEffect(() => {
       ...
       setIndex(0);
        ...
        {showAnswer === false ? <FrontCard props={{deleteCard, createCard, creatingCard, createCardPopup, deckInfo, index, duplicate, decreaseIndex, increaseIndex, showDefnintion}}/> : 
